...
public class GlobalRepositoryTools {
    ...
    public List<Map<String, Object>> topGrowingGlobals(String date) {
        ...
        List<Object[]> rows = em.createNativeQuery("""
                ...
                """)
                .setParameter("dt", LocalDate.parse(snapshotDate.toString()))
                .getResultList();

        return rows.stream()
                .map(r -> {
                    Map<String, Object> m = new HashMap<>();
                    m.put("globalName", r[0]);
                    m.put("growthMB", r[1]);
                    m.put("growthPct", r[2]);
                    m.put("usedMB", r[3]);
                    m.put("location", r[4]);
    ...
    @Tool("""
            ...
            RULES:
            - Only call this tool if one of the thresholds are explicitly known.
    ...
    @Tool("""
            ...
            - When the user needs more information about a specific global's growth history
            - When the user asks about which table the global is related to
